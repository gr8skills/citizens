...
class EloquentCitizen implements CitizenRepository
{
    ...
    public function associateSocialAccountForCitizen($citizenId, $provider, SocialUser $citizen)
    {
        return DB::table('social_logins')->insert([
            'user_id' => $citizenId,
            'provider' => $provider,
            'provider_id' => $citizen->getId(),
            'avatar' => $this->getAvatarForProvider($provider, $citizen),
    ...
    {
        return Citizen::whereBetween('created_at', [Carbon::now()->firstOfMonth(), Carbon::now()])
            ->count();
    }
    ...
    {
        $result = Citizen::whereBetween('created_at', [$from, $to])
            ->orderBy('created_at')
            ->get(['created_at'])
