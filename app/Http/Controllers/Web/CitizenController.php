...
use Vanguard\Repositories\User\CitizenRepository;
use Vanguard\Repositories\User\UserRepository;
use Vanguard\Services\Upload\UserAvatarManager;
use Vanguard\Support\Enum\CitizenStatus;
...
class CitizenController extends Controller
{
    ...
    /**
     * CitizensController constructor.
     * @param CitizenRepository $citizens
     */
    public function __construct(CitizenRepository $citizens)
    {
        $this->middleware('auth');
        $this->middleware('session.database', ['only' => ['sessions', 'invalidateSession']]);
//        $this->middleware('permission:users.manage');

        $this->middleware('permission:citizen.view', ['only' => 'show']);
        $this->middleware('permission:citizen.edit', ['only' => 'edit']);
        $this->middleware('permission:citizen.delete', ['only' => 'destroy']);
        $this->middleware('permission:citizen.create', ['only' => 'create']);
    ...
    {
        $citizens = $this->citizens->paginate(
            $perPage = 20,
            Input::get('search'),
            Input::get('status')
        );

        $statuses = ['' => trans('app.all')] + CitizenStatus::lists();

        return view('citizen.list', compact('citizens', 'statuses'));
    }
